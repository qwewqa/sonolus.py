...
An int-keyed open-addressing hash table backs the optimizer's const-interning
map (f64 bit pattern -> const id) -- a flat array with far less per-element
overhead than a Python dict, and error codes instead of C++ ``bad_alloc`` so
callers stay ``noexcept`` and raise ``MemoryError`` on OOM.
