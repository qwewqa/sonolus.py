...
cdef class _Cleaner:
    ...
    # making one iteration O(nb + ne) rather than a naive O(nb * ne) rescan.
    # out_first/out_last bucket alive+stale edge ids by e_src; in_first/in_last by
    # e_dst. Entries are never removed: a kill (e_alive=0) or a redirect (e_src/e_dst
    # rewritten, with a fresh node appended to the new head's list) just leaves a
    # stale node behind, which consumers skip by re-checking e_alive and
    # e_src/e_dst == h. Appending at the tail keeps each head's live edges in
    # ascending edge-index order (merge/taildup only ever add edges to a
    ...
    cdef list _rpo_order(self):
        ...
        # never overflow the C call stack. Postorder: successors visited in
    ...
    cdef void run(self) except *:
        ...
        # one iteration is O(nb + ne) rather than a naive O(nb * ne) full-edge-array
        # rescan.
    ...
    cdef int32_t _copy_place(self, Func dst, int32_t pid, int32_t k) except -1:
        ...
        return _add_place(dst, p.kind, p.flags, new_block_ref, new_index_val, p.offset)
...
cdef class _SSABuilder:
    ...
    cdef void _run(self):
        # The Braun read path is explicit-stack iteration (_begin_read/_drain), so
        # deep CFGs cannot overflow the C stack.
        cdef int32_t b, s
...
cdef class _UnSSA:
    ...
    def _analyze(self):
        cdef Func src = self.src
        cdef int32_t i, b, op, k, astart, nargs, e
    ...
    def _plan(self):
        ...
        # value. A pure self-loop's test is control-dead, so splitting it is only
        # a normalization keeping verify() and lowering uniform.
        self.n_out = self.nb
        for b in range(src.n_blocks):
...
cdef inline uint64_t _dbits(double d) noexcept nogil:
    ...
    # bitwise (preventing NaN-through-a-loop-phi nontermination) and -0.0 stays
...
cdef class _SCCP:
    ...
    cdef object _transfer(self, int32_t i):
        ...
        saw_top = False
        nc = 0
        # 16-slot stack fast path; heap-fallback for wider variadic ops (large
        # Switch*) so >16-arg instances still fold rather than going BOTTOM.
        buf = stackbuf
        if n > 16:
            buf = <double*>malloc(<size_t>n * sizeof(double))
            if buf == NULL:
                return (_LAT_BOTTOM, 0.0, None)
        # try/finally so the heap buffer is always freed even if boxing r or
        # building the result tuple raises (mirrors kernels.pyx fold()).
        try:
            for k in range(n):
                a = <int32_t>f.args[astart + k]
                ...
                if kk == _LAT_BOTTOM or kk == _LAT_SET:
                    ...
                    nc += 1
            if saw_bottom:
                result = (_LAT_BOTTOM, 0.0, None)
            elif saw_top:
                result = (_LAT_TOP, 0.0, None)
            elif op < OP_RUNTIME_COUNT and SONOLUS_OP_FOLDABLE[op] != 0:
                status = fold_op(<uint16_t>op, buf, n, &r)
                if status == FOLD_OK:
                    result = (_LAT_CONST, r, None)
                else:
                    result = (_LAT_BOTTOM, 0.0, None)
            else:
                result = (_LAT_BOTTOM, 0.0, None)
...
def _run_gvn_inplace(Func f):
    ...
    while work:
        frame = work.pop()
        if frame[0] == "exit":
            # Unwind this dominator scope: GVN inserts a key only when it was absent,
            # so every undo entry is a fresh insertion -- unwinding always deletes it.
            for key in <list>frame[1]:
                del avail[key]
            continue
        b = <int32_t>frame[1]
        undo = []
...
#
# Both reshape the CFG on SSA form (phis live). ``rewrite_switch`` rewrites block
# tests + edge conds/targets, drops dead blocks, and relocates escaping consts to
# entry; LICM relocates instructions into a fresh preheader block. Both rebuild
# through the general model-driven rebuilder ``_emit_from_model`` below.
# ==========================================================================
...
def _emit_from_model(Func src, list pblocks, int entry_pb):
    ...
    for k in range(nb_new):
        ...
        has_value_edge = False
        for espec in <list>(<dict>pblocks[pb])["edges"]:
            if <int32_t>espec[1] == EDGE_COND_VALUE:
                has_value_edge = True
                break
...
cdef list _incoming_by_block(Func f):
    # incoming[b] = ascending-global-edge-index list of edges with dst == b (the
    # phi operand <-> incoming-edge contract order).
    cdef list incoming = [[] for _ in range(f.n_blocks)]
    cdef int32_t e
    for e in range(f.n_edges):
        (<list>incoming[f.edges[e].dst]).append(e)
    return incoming


def _phi_opmap_src(Func f, int32_t vid, list incoming):
    ...
    cdef int32_t k
    cdef list inc = <list>incoming[f.instrs[vid].block]
    opmap = {}
    for k in range(nargs):
...
def _licm_try_loop(Func f, Dominators D, LoopForest F, int32_t L):
    ...
    cdef int32_t w, bit, base, istart, icount
    cdef int64_t body_off
    cdef uint64_t word
    if header == entry:
        return None
    ...
    # via the arena layout contract (ir.pxd: block instr slices partition the
    # stream in block-id order, ids == RPO); a producer violating that would only
    # cause missed hoists (unvisited operand -> non-invariant), never an unsound one.
    inv = {}
    body_off = <int64_t>L * F.nwb
...
# via the general model builder _emit_from_model, rebuilding tests + edges and
# relocating escaping consts to entry.
...
# Pointed/Shifted ops, so this currently never fires -- it is guarded by a
# cheap pre-scan so the common (no-such-op) case allocates nothing.
# --------------------------------------------------------------------------
...
def _midend_pass(Func func):
    f1, c1 = _run_sccp(func)
    _, c2 = _run_gvn_inplace(<Func>f1)
    cb = _canon_branch_not(<Func>f1)
    cf = _fuse_ptr_rmw(<Func>f1)
    # DCE last: reaps the orphaned GetPointed/BinOp and the freed Not from the
    # passes above.
