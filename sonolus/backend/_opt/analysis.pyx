...
cdef Liveness compute_liveness(Func func):
    ...
    with nogil:
        ...
        # The dataflow has a unique fixpoint. An exit-reachable block that can reach
        # an exit-unreachable region gains that region's uses in its live_out (so
        # stores feeding them are not DSE'd), as do the exit-unreachable blocks
        # themselves.
...
def liveness_debug(entry, mode=None, callback=None):
    ...
    Returns ``{"live_in", "live_out", "stmt_live", "array_defs_out",
    "is_array_init", "n_blocks"}`` where the block keys are arena block ids
    (reverse-postorder, matching ``traverse_cfg_reverse_postorder(entry)``) and
    values are sets of the original source temp names. ``stmt_live`` maps
    statement-root instr indices to their live-out sets; ``is_array_init`` maps
    OPX_SET instr indices to whether that write is the array's first.
    """
