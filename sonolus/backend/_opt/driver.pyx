...
pipelines. The authoritative per-level pipeline description lives on ``_pipeline``.
...
registering the pipeline phases (``cfg_cleanup``, ``ssa``, ``unssa``, ``ifconv``,
``lower``, ``dominators``) and the three allocators (``bump``, ``packing``,
``try_bump``) so ``debug_run(cfg, phases=["cfg_cleanup"])`` runs a single named phase.
"""
...
cdef Func _pipeline(Func func, int level, bint allocate):
    ...
    Single authoritative description of what each level runs:
    ...
    ``midend_standard`` runs that core plus LICM and rewrite_switch, repeating
    the core once on change. ``if_convert`` (standard only) folds
    diamonds/triangles/{VALUE C, NONE} blocks into ``If`` expression selects over
    SSA while phis still exist, relying on must-fold flags that ``lower_from_ssa``
    honors. ``lower_from_ssa`` is the real out-of-SSA + treeify + coalesce +
