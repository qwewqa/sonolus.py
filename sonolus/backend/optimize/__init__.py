...
* ``OptimizationLevel`` -- opaque level sentinel (a name; no ordering).
* ``MINIMAL_PASSES`` / ``FAST_PASSES`` / ``STANDARD_PASSES`` -- the -O0/-O1/-O2 levels.
* ``OptimizerConfig(mode, callback)`` -- marshal-in context (writability resolution).
* ``run_passes(entry, level, config=None, *, allocate=True) -> BasicBlock``.
* ``optimize_and_finalize(entry, level, config=None) -> EngineNode`` (fused).
* ``cfg_to_engine_node(entry) -> EngineNode`` -- emit only, no passes.
"""
...
def _level_name(level: OptimizationLevel) -> str:
    ...
    if name is None:
        raise ValueError(
            f"Invalid optimization level {level!r}; expected one of "
            f"MINIMAL_PASSES, FAST_PASSES, STANDARD_PASSES"
        )
    return name
