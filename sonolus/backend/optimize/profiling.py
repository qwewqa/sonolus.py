...
work (M9 Part B) measured against real numbers.
...
the timing calls entirely, so an ordinary build pays only one bint test per stage.
...
# Read once at import, like SONOLUS_OPT_TRACE; the CLI can also flip it via enable().
