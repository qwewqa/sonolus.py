"""M3 rewrite_switch tests (OPTIMIZER_REWRITE.md 7.2.6 / section 4).

Mirrors the old ``RewriteToSwitch``:
...
def _assert_semantics(build, mode=Mode.PLAY, cb=None, seed=None):
    ref = _run_node(cfg_to_engine_node(run_passes(build(), MINIMAL_PASSES, OptimizerConfig(mode=mode, callback=cb))), seed)
    ...
    for blk in blocks:
        for i in range(32):
            ...
            assert a == b or (a == 0.0 and b == 0.0), f"memory mismatch {blk}[{i}]: {a} vs {b}"
    return ref, std
...
def test_corpus_standard_midend_pipeline(mode):
    ...
    for label, cb, factory in _iter_callbacks(mode):
        # fast (M2) baseline vs standard (M3: + LICM + rewrite_switch). verify()
