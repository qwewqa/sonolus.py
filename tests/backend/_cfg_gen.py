...
Mirrors the shallow three-address CFGs the frontend emits: scalar/array ``TempBlock`` virtual registers, plain int memory blocks
...
identical program as many times as they need.
...
@dataclass(frozen=True)
class IfVal:
    """A strict-select value ``If(test, then_, else_)``.

    Per the frontend contract, only the test may be a non-constant expression;
    both arms are single constants.
    """

    test: object  # any expr recipe (may be non-constant)
    then_: float  # constant
    else_: float  # constant


@dataclass(frozen=True)
class MembSwitch:
    """A ``x in S`` membership ``SwitchWithDefault(test, k0, v0, ..., default)``.

    Only the test may be non-constant; keys, per-key values, and the default are
    all single constants (matching the decided frontend contract).
    """

    test: object  # any expr recipe (may be non-constant)
    keys: tuple  # distinct constant int keys
    vals: tuple  # one constant value per key
...
class Rmw:
    ...
    This is the shape the fusion peephole targets. ``target`` selects the
...
def _expr(draw, n_scalars, array_sizes, depth):
    ...
    kind = draw(st.sampled_from(["leaf", "leaf", "unary", "binary", "ifval", "membswitch"]))
    if kind == "leaf":
        return draw(_leaf(n_scalars, array_sizes))
    if kind == "unary":
        return Unary(draw(st.sampled_from(_UNARY_OPS)), draw(_expr(n_scalars, array_sizes, depth - 1)))
    if kind == "ifval":
        # Strict-select value If: only the test is a (possibly non-constant) expr;
        # both arms are single constants (the frontend contract, oracle-total).
        return IfVal(
            draw(_expr(n_scalars, array_sizes, depth - 1)),
            draw(st.sampled_from(_CONSTS)),
            draw(st.sampled_from(_CONSTS)),
        )
    if kind == "membswitch":
        # Membership SwitchWithDefault (x in S): distinct constant int keys, a
        # constant value per key, and a constant default; only the test is an expr.
        keys = tuple(sorted(draw(st.sets(st.integers(-4, 4), min_size=1, max_size=4))))
        vals = tuple(draw(st.sampled_from(_CONSTS)) for _ in keys)
        return MembSwitch(
            draw(_expr(n_scalars, array_sizes, depth - 1)),
            keys,
            vals,
...
class _Builder:
    ...
    def expr(self, e) -> IRExpr:
        match e:
            ...
            case IfVal(test, then_, else_):
                return IRPureInstr(Op.If, [self.expr(test), IRConst(then_), IRConst(else_)])
            case MembSwitch(test, keys, vals, default):
                args: list = [self.expr(test)]
                for k, v in zip(keys, vals, strict=True):
                    args += [IRConst(k), IRConst(v)]
                args.append(IRConst(default))
                return IRPureInstr(Op.SwitchWithDefault, args)
