...
The Multiply x0 / x+0 rewrites legally collapse -0.0 -> +0.0 (a policy
...
def test_sccp_defaultless_nonexhaustive_switch_missed_element_exits():
    ...
    for wsel, expect in [(0, [-2, 100, -1]), (1, [-2, 101, -1]), (7, [-2])]:
        ...
        mid = _run_seeded(
            cfg_to_engine_node(
                run_passes(midend.run_midend(build(), None, None, allow_repeat=True), MINIMAL_PASSES, OptimizerConfig())
            ),
            wsel,
        )
...
def test_sccp_switch_on_degenerate_constant_takes_default():
    ...
    assert "DebugLog(1)" not in text  # cases pruned
    assert "DebugLog(2)" not in text
...
# ==========================================================================
# SCCP folding of the strict-select value ops (If / Switch*). These carry
# control_flow=True yet fold as strict selects over all-constant operands.
# ==========================================================================


def test_sccp_folds_all_const_selects():
    # All-constant If/Switch/SwitchInteger/SwitchWithDefault fold to their selected
    # arm, including a >16-arg SwitchWithDefault (22 args -> the heap-fallback path).
    def build():
        pairs = []
        for k in range(10):
            pairs += [IRConst(k), IRConst(100 + k)]
        big_switch = IRPureInstr(Op.SwitchWithDefault, [IRConst(9), *pairs, IRConst(999)])  # 22 args -> 109
        b0 = BasicBlock(
            statements=[
                _log(IRPureInstr(Op.If, [IRConst(1), IRConst(111), IRConst(222)])),  # 111
                _log(IRPureInstr(Op.If, [IRConst(0), IRConst(111), IRConst(222)])),  # 222
                _log(IRPureInstr(Op.Switch, [IRConst(2), IRConst(1), IRConst(11), IRConst(2), IRConst(22)])),  # 22
                _log(IRPureInstr(Op.SwitchInteger, [IRConst(1), IRConst(70), IRConst(80), IRConst(90)])),  # 80
                _log(big_switch),
            ]
        )
        b0.connect_to(BasicBlock(), None)
    ...
    text = _text(build(), ["cfg_cleanup", "ssa", "sccp", "dce"])
    assert "If(" not in text  # every select folded to a constant
    assert "Switch" not in text
    it = _interp_midend(build)
    assert it.log == [111.0, 222.0, 22.0, 80.0, 109.0]


def test_sccp_nonconst_select_discriminant_stays_bottom():
    # A non-constant test keeps the select at BOTTOM: it is not folded, survives to
    # emit, and lazily evaluates the selected arm at runtime.
    def build():
        b0 = BasicBlock(statements=[_log(IRPureInstr(Op.If, [_w(0), IRConst(111), IRConst(222)]))])
        b0.connect_to(BasicBlock(), None)
        return b0

    text = _text(build(), ["cfg_cleanup", "ssa", "sccp", "dce"])
    assert "If(" in text  # non-const discriminant -> not folded (BOTTOM)
    _assert_semantics(build)  # MINIMAL vs midend agree (W[0] undefined -> -1.0, truthy -> 111)


def test_sccp_folded_select_feeding_test_prunes_edges():
    # A value-If that folds to a constant feeds a block test; SCCP evaluates it and
    # prunes the now-dead edge, exactly like a constant test.
    def build():
        head = BasicBlock(test=IRPureInstr(Op.If, [IRConst(1), IRConst(0), IRConst(5)]))  # -> 0
        tb = BasicBlock(statements=[_log(111)])
        fb = BasicBlock(statements=[_log(222)])
        join = BasicBlock(statements=[_log(999)])
        head.connect_to(fb, 0)  # cond-0 edge taken (test folds to 0)
        head.connect_to(tb, None)
        tb.connect_to(join, None)
        fb.connect_to(join, None)
        return head

    text = _text(build(), ["cfg_cleanup", "ssa", "sccp", "dce"])
    assert "111" not in text  # true arm pruned (folded test == 0 takes the cond-0 edge)
    assert "222" in text
    it = _assert_semantics(build)
    assert it.log == [222.0, 999.0]


def test_sccp_folded_select_negative_zero_follows_const_policy():
    # A value-If folding to -0.0 follows the const_override -0.0 skip: it is NOT
    # replaced by a bare constant (which would ship as +0.0 across marshal/emit) --
    # the instruction survives so the runtime recomputes the sign. The lattice keeps
    # -0.0 (Arctan2(0, -0) folds to pi, not 0), mirroring the folded-arithmetic case.
    def build_store():
        sel = IRPureInstr(Op.If, [IRConst(1), IRPureInstr(Op.Negate, [IRConst(0.0)]), IRConst(5)])
        b0 = BasicBlock(statements=[IRSet(BlockPlace(W, 0), sel)])
        b0.connect_to(BasicBlock(), None)
        return b0

    assert "If(" in _text(build_store(), ["cfg_cleanup", "ssa", "sccp", "dce"])  # not materialized

    def build_probe():
        sel = IRPureInstr(Op.If, [IRConst(1), IRPureInstr(Op.Negate, [IRConst(0.0)]), IRConst(5)])
        b0 = BasicBlock(statements=[_log(IRPureInstr(Op.Arctan2, [IRConst(0.0), sel]))])
        b0.connect_to(BasicBlock(), None)
        return b0

    assert "Arctan2" not in _text(build_probe(), ["cfg_cleanup", "ssa", "sccp"])  # folded with -0.0
    it = _interp_midend(build_probe)
    assert abs(it.log[0] - math.pi) < 1e-9


def test_shared_guarded_divide_strictness_flip_documented_illegal():
    # DOCUMENTED-ILLEGAL SHAPE (not endorsed): two value-Ifs guarding bit-identical
    # Divide(1, t) arms that share one SSA value (t routed through a scalar temp).
    # This VIOLATES the strict-select / oracle-total arm-legality contract (a
    # Divide(1, 0) arm raises in the f64 oracle), so the frontend must never emit it.
    # The test pins the KNOWN divergence: MINIMAL keeps the arms lazy (guard false ->
    # arm unevaluated), but FAST/STANDARD dedup the shared Divide and treeify
    # materializes it ABOVE the selects, so the oracle eagerly computes 1/0 and
    ...
    def build():
        ...
        b0 = BasicBlock(
            statements=[
                ...
                _log(IRPureInstr(Op.If, [_rd("t"), div(), IRConst(100)])),  # guard t: t!=0 ? 1/t : 100
                _log(IRPureInstr(Op.If, [_rd("t"), div(), IRConst(200)])),
            ]
        )
        b0.connect_to(BasicBlock(), None)
        return b0
    ...
    def run_at(level):
        ...
        it = Interpreter()
        it.blocks[3000] = list(_ROM)
        it.blocks[W] = [0.0]  # t == 0 at runtime
        it.run(node)
        return it

    # MINIMAL: lazy arms, guard false -> Divide(1, 0) never evaluated.
    assert run_at(MINIMAL_PASSES).log == [100.0, 200.0]
    # FAST and STANDARD: the shared Divide is hoisted above the select and evaluated
    # eagerly -> the f64 oracle raises (the documented strictness flip at -O1 and -O2).
...
def test_gvn_readonly_get_unified_writable_never():
    ...
    def build_w():
        b0 = BasicBlock(statements=[_log(_w(0)), _log(_w(0)), IRInstr(Op.DebugPause, [IRConst(0)])])
        b0.connect_to(BasicBlock(), None)
        return b0
...
def test_dce_used_by_dead_phi_only_deleted():
    ...
    def build():
        ...
        tb = BasicBlock(
            statements=[IRSet(_sc("x"), IRPureInstr(Op.Cosh, [_w(1)])), IRInstr(Op.DebugPause, [IRConst(0)])]
        )
        fb = BasicBlock(
            statements=[IRSet(_sc("x"), IRPureInstr(Op.Sinh, [_w(1)])), IRInstr(Op.DebugPause, [IRConst(1)])]
        )
...
# ==========================================================================
# Round driver: one SCCP round folds the branch cascade; allow_repeat is a
# defensive fixpoint re-run, not required by this case.
# ==========================================================================


def test_midend_round_folds_branch_cascade():
    # One SCCP round (conditional constant propagation) folds the constant branch,
    # the realigned phi, and the downstream Add in a single pass. midend_round's
    # allow_repeat re-run is a defensive fixpoint pass, not required here (the
    # allow_repeat=False and allow_repeat=True texts are identical, asserted below).
    def build():
    ...
    # Structural pin: the whole cascade folds to a single DebugLog(15), with no
    # surviving phi or Add left for a second round to clean up.
    text = _text(build(), ["cfg_cleanup", "ssa", "midend"])
    assert "DebugLog(15)" in text
    assert "phi" not in text
    assert " + " not in text
    # One round suffices: the defensive repeat pass is a no-op on this case.
    assert cfg_to_text(midend.run_midend(build(), None, None, allow_repeat=False)) == cfg_to_text(
        midend.run_midend(build(), None, None, allow_repeat=True)
    )
...
def _f(x: float) -> bytes:
    # +-0.0 folded to a single key (a policy tolerance); NaN keeps its
    # bit pattern.
    if x == 0.0:
