...
def slot_count(cfg):
    """max(effective offset)+1 over all emitted block-10000 places."""
    mx = 0
...
def test_conditional_infinite_loop_store_survives():
    ...
    # so packing's dead-store elimination must NOT drop it (if the spin block is
    # never visited, x looks dead and its store gets clobbered).
    def make():
        x = _sc("x")
        b0 = BasicBlock()
        spin = BasicBlock()
...
def test_pydori_corpus_packing():
    total_slots = 0
    n = 0
    for name, cfg, mode, callback in _enumerate_pydori_callbacks():
        ...
        cfg = ir.to_basic_blocks(func)
        slots = slot_count(cfg)
        # Packing must stay within the 4096-slot temp-memory cap (block 10000).
        assert slots <= 4096, f"{name}: {slots} slots exceeds cap"
        total_slots += slots

    assert n > 0
    print(f"\n[pydori corpus] {n} callbacks: packing slots total={total_slots}")
