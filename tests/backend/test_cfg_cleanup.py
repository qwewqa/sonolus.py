"""Tests for the ``cfg_cleanup`` mid-end pass.
...
2. Semantic differential on hand-built interpretable CFGs: the ORIGINAL and the
   CLEANED CFG, each run through ``run_allocate`` (bump) ->
...
from tests.backend._corpus import iter_callbacks
...
def _interp(entry, seed):
    # Allocate temps (bump) then emit via the arena emitter, so the original and
    # cleaned CFGs are compared through the same toolchain.
...
def test_corpus_cleanup_properties(mode):
    ...
    for label, callback_name, factory in iter_callbacks(mode):
        ...
        out = ir.to_basic_blocks(cleaned)
    ...
    # dispatches (a deliberate tradeoff).
