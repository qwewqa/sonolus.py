...
* For each program the CFG is *regenerated* fresh for every use so each pipeline
  run sees an independent CFG (defensive isolation; the public API is documented
  non-destructive).
* Observables captured after interpretation: the return value, the ``DebugLog``
  ...
  by their IEEE-754 bytes (``struct.pack(">d", ...)``), so NaN drift is caught
  bit-exactly while +/-0.0 compare as equal (per the policy exceptions in ``_f``).
...
def _f(x: float) -> bytes:
    ...
    # zero relative to the MINIMAL reference via two *policy exceptions*:
...
def test_generated_programs_are_small(program):
    # Structural size envelope: the strategy's maximum is entry + B(3), where
    # B(d) = 6 + 5*B(d-1) and B(0) = 0 -- the all-switch worst case (4 contiguous
    # conds + default = 5 arms, plus test/join blocks per level). B(3) = 186, so
    # the bound is 187. An invariant, not an empirical percentile.
    assert count_blocks(build_cfg(program)) <= 187
...
A, B = OBS_BLOCKS  # the two observable int blocks the harness captures
...
def test_directed_undef_loop_carried_dead_sibling():
    ...
    # the def-point interference graph must include the s_obs<->s_dead edge; if it
    # omits it, coalescing fuses two independent variables. s_obs (observed) then gets
