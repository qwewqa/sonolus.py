...
# ---------------------------------------------------------------------------
# THE key semantic test: a guarded division must fold UNDER the If.
# ---------------------------------------------------------------------------
...
def test_guarded_division_folds_under_if_no_fault():
    ...
    # 3) the non-if-converted path (a real branch) agrees. Its only If is the
...
def _triangle(true_direct: bool):
    ...
    head = BasicBlock(statements=[IRSet(_sc("a"), _in(0)), IRSet(_sc("d"), IRPureInstr(Op.Abs, [_in(2)]))], test=_in(3))
...
# ---------------------------------------------------------------------------
# Arm budget (per select arm tree, effective cost <= 8).
# ---------------------------------------------------------------------------
...
def test_arm_budget_boundary():
    ...
    # (Multiply avoids the GVN Subtract/Add<->Negate recombination, which would
    # otherwise fold Subtract(x, Negate(y)) -> Add(x, y) pre-ifconv.)
    def c8():
...
def _multiway_block():
    # A >=3-way switch head is not a two-way head, so it is not converted. It must
...
def test_corpus_if_convert(mode: Mode):
    ...
    for _label, cbname, factory in iter_callbacks(mode):
        ...
        eff_conv += _effective_count(node, mode, cbname)
        eff_base += _effective_count(base, mode, cbname)
        count += 1
    assert count > 0
...
def _f(x: float) -> bytes:
    ...
    # of zero relative to the MINIMAL reference (e.g. SCCP folds ``Negate(Not(k))``
    # to +0.0 where the unoptimized tree evaluates to -0.0). Mirrors test_random_cfg.py's
    # _f; NaN bytes pass through unchanged (NaN != 0.0), so NaN drift is still caught.
    x = float(x)
    if x == 0.0:
        x = 0.0
