...
def _assert_semantics(build, mode=Mode.PLAY, cb=None, seed=None):
    ...
    for blk in blocks:
        for i in range(32):
            a = ref.get(blk, i)
            b = std.get(blk, i)
            assert a == b or (a == 0.0 and b == 0.0), f"memory mismatch block {blk}[{i}]: {a} vs {b}"
    return ref, std
...
def test_conditionally_executed_not_hoisted():
    ...
    def build():
        ...
        then = BasicBlock(
            statements=[IRSet(_sc("acc"), IRPureInstr(Op.Add, [_rd("acc"), IRPureInstr(Op.Multiply, [_ru(0), _ru(1)])]))]
        )
